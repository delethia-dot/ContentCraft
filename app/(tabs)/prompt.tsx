...
const AI_TOOLS: { id: AITool; label: string; mediaTypes: MediaType[]; color: string; description: string }[] = [
  { id: "midjourney", label: "Midjourney", mediaTypes: ["image"], color: "#7C3AED", description: "Artistic & photorealistic images" },
  { id: "dalle", label: "DALL-E 3", mediaTypes: ["image"], color: "#10A37F", description: "OpenAI's image generator" },
  { id: "stable-diffusion", label: "Stable Diffusion", mediaTypes: ["image"], color: "#F97316", description: "Open-source image generation" },
  { id: "sora", label: "Sora", mediaTypes: ["video"], color: "#0EA5E9", description: "OpenAI's video generator" },
...
const ASPECT_RATIOS: Record<SocialPlatform, { label: string; value: string }[]> = {
  ...
  facebook: [{ label: "Landscape 16:9", value: "16:9" }, { label: "Square 1:1", value: "1:1" }, { label: "Portrait 4:5", value: "4:5" }],
  ...
  linkedin: [{ label: "Landscape 1.91:1", value: "1.91:1" }, { label: "Square 1:1", value: "1:1" }, { label: "Portrait 4:5", value: "4:5" }],
};
...
export default function PromptScreen() {
  ...
  const availableTools = AI_TOOLS.filter((t) => t.mediaTypes.includes(mediaType));
  const availableRatios = ASPECT_RATIOS[selectedPlatform];

  // Ensure selected tool is valid for current media type
  ...
  }, [subject, currentTool, mediaType, selectedStyle, selectedMood, selectedPlatform, selectedRatio, niche, additionalDetails, generateMutation]);
  ...
  const toolColor = AI_TOOLS.find((t) => t.id === currentTool.id)?.color ?? colors.primary;
        ...
        <View style={styles.content}>
            ...
            <View style={[styles.toggleRow, { backgroundColor: colors.background, borderColor: colors.border }]}>
              {(["image", "video"] as MediaType[]).map((type) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setMediaType(type);
                    const firstTool = AI_TOOLS.find((t) => t.mediaTypes.includes(type));
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      {
                        backgroundColor: isActive ? p.color : colors.background,
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.chip,
                      {
                        backgroundColor: isActive ? "#B8860B" : colors.background,
                        borderColor: isActive ? "#B8860B" : colors.border,
                      },
                    ]}
                  >
                    <Text style={[styles.chipText, { color: isActive ? "#FFFFFF" : colors.foreground }]}>
          ...
          <View style={[styles.section, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.sectionHint, { color: colors.muted }]}>
              Describe what you want in the {mediaType} (e.g., "a woman doing yoga on a rooftop at sunrise")
            </Text>
            <TextInput
              value={subject}
          ...
          <View style={[styles.section, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            <Text style={[styles.sectionHint, { color: colors.muted }]}>
              Optional: lighting, colors, camera angle, specific elements to include or avoid
            </Text>
            <TextInput
              value={additionalDetails}
              onChangeText={setAdditionalDetails}
              placeholder="e.g., golden hour lighting, shallow depth of field, no text..."
              placeholderTextColor={colors.muted}
              multiline
              numberOfLines={2}
              returnKeyType="done"
          ...
          <TouchableOpacity
            ...
            style={[
              styles.generateBtn,
              { backgroundColor: isGenerating ? colors.muted : toolColor },
            ]}
          >
            {isGenerating ? (
              <>
              ...
              <>
                ...
                <Text style={styles.generateBtnText}>
                  Generate {currentTool.label} Prompt
                </Text>
              </>
            )}
          </TouchableOpacity>
          ...
          {/* Result */}
          {generatedPrompt && (
            <View style={[styles.resultCard, { backgroundColor: colors.surface, borderColor: toolColor + "40" }]}>
              {/* Result Header */}
              <View style={[styles.resultHeader, { backgroundColor: toolColor + "12" }]}>
                <View style={[styles.toolDot, { backgroundColor: toolColor }]} />
                <Text style={[styles.resultHeaderText, { color: toolColor }]}>
                  {currentTool.label} Prompt Ready
                </Text>
                <TouchableOpacity onPress={sharePrompt} activeOpacity={0.7} style={styles.shareIconBtn}>
                  <IconSymbol name="square.and.arrow.up" size={18} color={toolColor} />
                </TouchableOpacity>
              </View>
              ...
              <View style={styles.promptBlock}>
                ...
                <View style={[styles.promptBox, { backgroundColor: colors.background, borderColor: toolColor + "30" }]}>
              ...
              <View style={[styles.qualityBox, { backgroundColor: "#10B98110", borderColor: "#10B98130" }]}>
                ...
                <Text style={[styles.qualityText, { color: colors.foreground }]}>
                  {generatedPrompt.estimatedQuality}
                </Text>
              </View>
                ...
                <View style={styles.promptBlock}>
                  ...
                  {generatedPrompt.tips.map((tip, i) => (
                    <View key={i} style={styles.tipRow}>
                      <View style={[styles.tipDot, { backgroundColor: toolColor }]} />
              ...
              <View style={styles.resultActions}>
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                  style={[styles.resultActionBtn, { backgroundColor: toolColor }]}
                >
                  <IconSymbol name="doc.on.doc" size={16} color="#FFFFFF" />
                  <Text style={styles.resultActionBtnText}>Copy Main Prompt</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={sharePrompt}
                  activeOpacity={0.8}
...
const styles = StyleSheet.create({
  ...
  resultActionBtnText: { fontSize: 14, fontWeight: "700", color: "#FFFFFF" },
});
